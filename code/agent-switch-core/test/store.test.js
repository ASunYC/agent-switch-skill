...
test("Store masks auth, persists, and reloads from disk", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-"));
...
test("listSessionsMulti merges roots and sorts newest session first", () => {
  const a = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-a-"));
  const b = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-b-"));
...
test("loadSessionMulti merges entries from both roots by ts", () => {
  const a = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-ma-"));
  const b = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-mb-"));
...
test("hasCapturedLogs ignores empty session directories", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-empty-"));
  const session = "2026-05-25T12-00-00-000Z";
...
test("loadSessionMulti keeps first root on equal ts and older mtime", () => {
  const a = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-tie-a-"));
  const b = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-tie-b-"));
  const session = "2026-05-25T12-00-00-000Z";
  const dirA = path.join(a, session);
  const dirB = path.join(b, session);
  fs.mkdirSync(dirA, { recursive: true });
...
test("loadSessionMulti picks newer file on equal ts (e.g. edited legacy)", () => {
  const a = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-edit-a-"));
  const b = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-edit-b-"));
  const session = "2026-05-25T12-00-00-000Z";
  const dirA = path.join(a, session);
  const dirB = path.join(b, session);
  fs.mkdirSync(dirA, { recursive: true });
...
test("loadSessionMulti prefers higher ts even when mtime is older", () => {
  const a = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-ts-a-"));
  const b = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-ts-b-"));
  const session = "2026-05-25T12-00-00-000Z";
  const dirA = path.join(a, session);
  const dirB = path.join(b, session);
  fs.mkdirSync(dirA, { recursive: true });
...
test("loadSessionMulti normalizes id from filename", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-key-"));
  const session = "2026-05-25T12-00-00-000Z";
...
test("loadSessionMulti skips corrupt json files", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-bad-json-"));
  const session = "2026-05-25T12-00-00-000Z";
  const dir = path.join(root, session);
  fs.mkdirSync(dir, { recursive: true });
...
test("loadSession skips corrupt json files", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-bad-load-"));
  const session = "2026-05-25T12-00-00-000Z";
  const dir = path.join(root, session);
  fs.mkdirSync(dir, { recursive: true });
...
test("readEntryById returns null for malformed id", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-bad-id-"));
  assert.equal(readEntryById(root, "not-valid"), null);
  fs.rmSync(root, { recursive: true, force: true });
});

test("readEntryByIdMulti prefers the newer duplicate across roots", () => {
  const a = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-ra-"));
  const b = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-rb-"));
  const session = "2026-05-25T12-00-00-000Z";
  const dirA = path.join(a, session);
  const dirB = path.join(b, session);
  fs.mkdirSync(dirA, { recursive: true });
...
test("Store writes a v2 manifest with blob refs; in-memory rec stays full", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-v2-"));
  const store = new Store({ root });
  const rec = store.add({
    request: {
...
test("v2 manifests are reconstructed transparently by loadSession/readEntryById", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-r2-"));
  const store = new Store({ root });
  const rec = store.add({
    request: {
...
test("legacy NNNN.json is repacked to v2 in place on read, idempotently", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-mig-"));
...
test("a corrupt v2 manifest yields null, not a thrown error", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-corrupt-"));
...
test("growing-prefix sessions store O(N) blobs, not O(N^2)", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-dedup-"));
  const store = new Store({ root });
  const N = 20;
  const messages = [];
