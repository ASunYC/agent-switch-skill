...
test("migrate copies legacy json into dest", () => {
  const proj = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-mig-fn-"));
  const dest = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-mig-fn-d-"));
  const session = "2026-05-25T12-00-00-000Z";
  const legacyDir = path.join(proj, ".agent-switch", session);
...
test("migrate exits 1 when legacy has no json logs", () => {
  const proj = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-mig-empty-"));
  const dest = fs.mkdtempSync(path.join(os.tmpdir(), "agent-switch-mig-empty-d-"));
  fs.mkdirSync(path.join(proj, ".agent-switch", "2026-05-25T12-00-00-000Z"), { recursive: true });
  assert.equal(hasCapturedLogs(path.join(proj, ".agent-switch")), false);
