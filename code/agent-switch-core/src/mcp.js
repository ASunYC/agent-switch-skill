// agent-switch MCP server: exposes the captured request/response logs to any MCP
...
//
// Run: node src/mcp.js   (root resolved from $AGENT_SWITCH_ROOT or global path)
...
const CWD = process.env.AGENT_SWITCH_CWD
  ? path.resolve(process.env.AGENT_SWITCH_CWD)
  : process.cwd();

const ROOT = process.env.AGENT_SWITCH_ROOT
  ? path.resolve(process.env.AGENT_SWITCH_ROOT)
  : globalRoot(CWD);
...
const server = new McpServer({ name: "agent-switch", version: "0.1.0" });
...
server.registerTool(
  "list_sessions",
  {
    ...
    description:
      "List all agent-switch capture sessions (newest first) with entry count, time range, and total USD cost.",
    inputSchema: {},
  },
  async () => {
