...
export function globalRoot(cwd) {
  return path.join(os.homedir(), ".agent-switch", "sessions", projectKey(cwd));
}

export function legacyRoot(cwd) {
  return path.join(cwd, ".agent-switch");
}
...
/** Write dir plus project-local ./.agent-switch (realpath-deduped). */
